...
newGraph3 <- ggplot(data = SNPs) + geom_bar(mapping = aes(x = chromosome, fill = genotype), position = "dodge") + facet_wrap(~ genotype, nrow = 10) +
             ggtitle("Ave SNP Count for each Human Chromosome") + xlab("Chromosome Name") + ylab("Total Number of SNPs")
newGraph3

## Exercise 5: Turn Lab3 ex6 into an interactive graph using plotly
newGraph4 <- ggplot(data = SNPs) + geom_bar(mapping = aes(x = chromosome, fill = genotype), position = "dodge") + facet_wrap(~ genotype, nrow = 3)
ggplotly(newGraph4)
